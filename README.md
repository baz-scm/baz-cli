...
- `--repo` and `--pr` are optional. When omitted, the CLI fetches open PRs across the repos you can access and reviews the most recently updated PR per repository. Env vars `BAZ_REPO` and `BAZ_PR_NUMBER` can still pre-select a target.
...
JSON output returns an array of review results when multiple repositories are considered; markdown output concatenates each repo's report with separators.
